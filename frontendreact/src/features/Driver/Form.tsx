...
const driverSchema = z.object({
  ...
  cnhType: z.string().min(1, "Tipo de CNH é obrigatório"),
...
const DriverForm: React.FC<DriverFormProps> = ({ formData, handleInputChange, handleSubmit }) => {
  ...
  useEffect(() => {
    try {
      ...
      driverSchema.parse(dataToValidate);
      setIsFormValid(true);
      setErrors({});
    } catch (error: any) {
      ...
      <div className="mb-4">
        ...
        <div className="row g-3 mt-1">
          ...
          <div className="col-md-3">
            ...
            <input
              type="text"
              className={`form-control ${shouldShowError('cnh') ? 'is-invalid' : ''}`}
              id="cnh"
              value={formData.cnh}
              onChange={handleNumericInput}
              ...
              maxLength={11}
            />
            {shouldShowError('cnh') && <div className="error-feedback">{errors.cnh}</div>}
          </div>
          <div className="col-md-3">
            <label className="form-label small">Tipo de CNH</label>
            <select
              className={`form-control ${shouldShowError('cnhType') ? 'is-invalid' : ''}`}
              id="cnhType"
              value={formData.cnhType}
              onChange={(e) => handleFieldChange(e.target.id, e.target.value)}
              onBlur={() => handleBlur('cnhType')}
              required
            >
            ...
            {shouldShowError('cnhType') && <div className="error-feedback">{errors.cnhType}</div>}
          </div>
        </div>
      </div>
      <hr className="my-4" />
